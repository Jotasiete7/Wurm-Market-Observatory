...
const seller = await FileAttachment(`data/${server.value.toLowerCase()}-seller-activity.json`).json();
const buyer  = await FileAttachment(`data/${server.value.toLowerCase()}-buyer-activity.json`).json();
```
...
with open('src/index.md', 'w', encoding='utf-8', newline='\r\n') as f:
    f.write(content)
print("index.md updated with Multi-Server support")
